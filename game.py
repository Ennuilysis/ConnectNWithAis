    ...
    def win_check(self, piece, name) -> bool:
        ...
        def check_next(row: int, col: int, row_chng: int, col_chng: int) -> int:
            if row == -1 or col == -1:
                return 0
            try:
                next_chr: str = board_list[row][col]from ConnectNGame.src.board import Board
    ...
    def play(self):
        while True:
            ...
            self.Player_instants.append(
                type_choices[type_choice].create_player(self.player_num, self.players, self.board))
            x = self.Player_instants[-1]
            self.players.append((x.name, x.piece, x.player_num))
            type_choice = input(f"Choose the type for Player {self.player_num}\rEnter Human or Random or Simple:")
            self.Player_instants.append(
